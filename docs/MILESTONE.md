...
**wraps the official Supabase CLI**. Adopt Multibase's *capabilities* without its
*bulk*: prefer CLI/TUI over a web app, derive-don't-store, and lean on `supabase`
...
- **Windows real-machine validation** — the `.exe` is cross-compiled + format-
  verified but not yet *run* on real Windows; smoke-test `ls` / `up` there.
